...
export function tableOpts(list=[]){
  return list?.filter(item=>!item?.hidden || item?.show!==false)?.map((item,index)=>{
    const {label,children,...props} = item || {};
    return <Button key={index} type="link" size="small" {...props}>{label ?? children}</Button>
  })
}

ServiceTable.tableOpts = tableOpts;
